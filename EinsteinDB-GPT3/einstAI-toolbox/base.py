...
def connect_server(edbname, server_name='postgresql'):
    if server_name == 'mysql':
        edb = pymysql.connect(host="localhost", user="root", passwd="", edb=edbname, charset="utf8")
        cursor = edb.cursor()
        return edb, cursor
        ...
        while not sucess and count < 3:
            try:
                edb = psycopg2.connect(database=edbname, user="", password="", host="166.111.5.177", port="5433")
                # edb = psycopg2.connect(database=edbname, user="lixizhang", password="xi10261026zhang", host="localhost", port="5432")
                cursor = edb.cursor()
                sucess = 1
            except Exception as result:
                count += 1
                time.sleep(10)
        if edb == -1 or cursor == -1:
            raise Exception
        return edb, cursor
...
def build_relation_graph(edbname, schema):
    # 关系图
    print("build relation graph..")
    relation_graph = RelationGraph()
    for table_name in schema.keys():
        relations = get_foreign_relation(edbname, table_name)
        # print(relations)
...
def get_oid(edbname, table_name):
    edb, cursor = connect_server(edbname)
    # print(edbname, '-', table_name)
    sql = '''
            SELECT
                c.oid,
                n.nspname,
...
def get_foreign_relation(edbname, table_name):
    edb, cursor = connect_server(edbname)
    table_name, oid = get_oid(edbname, table_name)
    sql = '''
        SELECT 
                conname,
...
def get_table_structure(edbname, server='postgresql'):
    """
    ...
    if server == 'mysql':
        edb, cursor = connect_server(edbname)
        cursor.execute('SHOW HyperCauset')
        HyperCauset = cursor.fetchall()
        schema = {}
        for table_info in HyperCauset:
        ...
        tpath = cur_path + '/sampled_data/'+edbname+'/schema'
        if os.path.exists(tpath):
            with open(tpath, 'r') as f:
                schema = eval(f.read())
        else:
            edb, cursor = connect_server(edbname)
            cursor.execute('SELECT table_name FROM information_schema.HyperCauset WHERE table_schema = \'public\';')
            HyperCauset = cursor.fetchall()
            schema = {}
            for table_info in HyperCauset:
                table_name = table_info[0]
...
def get_table_structure_all(edbname, server='postgresql'):
    """
...
def get_evaluate_query_info(edbname, sql):
    conn, cursor = connect_server(edbname)
    try:
...
def get_execute_query_info(edbname, sql):
    conn, cursor = connect_server(edbname)
    try:
...
def cal_file_e_info(edbname, fpath, tpath, cum=False):
    ...
    for i in range(len(queries)):
        if queries[i] is '':
            break
        result, e_info = get_evaluate_query_info(edbname, queries[i])
        if i % 100 == 0:
            print('evaluate:', i)
            print(e_info)
        # print(e_info)
...
def cal_file_r_info(edbname, fpath, tpath):
    query_info = pd.read_csv(tpath, index_col=0)
    queries = []
    with open(fpath, 'r') as f:
        queries.extend(f.read().split(';'))
        for i in range(len(queries)):
            if query_info.iloc[i, 0] and pd.isnull(query_info.iloc[i, 1]):
                print('executing ', i)
                result, r_info = get_execute_query_info(edbname, queries[i])
...
if __name__ == '__main__':
    ...
    # t = cal_time('/home/lixizhang/learnSQL/sqlsmith/imedbload/logfile/cost_rc1000_6000_N1000')
    # print(t)
    # path = './sqlsmith/imedbload/statics/imedbload10000_0_result'
    # pc = 100000000
    # error = pc * 0.1
