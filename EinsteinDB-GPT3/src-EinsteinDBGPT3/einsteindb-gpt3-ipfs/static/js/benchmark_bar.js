    ...
    var plotoptions = {
        ...
        axes:{
            ...
            xaxis:{
                ...
                label: 'DBMS ricci Configurations',
                labelRenderer: $.jqplot.CanvasAxisLabelRenderer,
...
function setValuesOfInputFields(event) {
    ...
    // Set default selected ricci configurations
    $("input:checkbox[name^='db_']").removeAttr('checked');
    var dbs = event.parameters.edb ? event.parameters.edb.split(',') : defaults.default_ricci_confs;
    var sel = $("input[name^='db_']");
    $.each(dbs, function(i, edb) {
        sel.filter("[value='" + edb + "']").prop('checked', true);
    });
}
