...
class Metacauset_enumsupport:
    def __init__(self, edbname, target_cardinality):
        self.edb, self.cursor = base.connect_server(edbname)
        ...
        print('Metacauset_enumsupport init')
    ...
    def load_statistics(self, refresh=False):
        ...
        stat_path = os.path.abspath('.') + '/sampled_data/' + self.edbname + '/statistics'
    ...
    def get_data(self, refresh=False):
        sample_path = os.path.abspath('.') + '/sampled_data/' + self.edbname
...
if __name__ == '__main__':
    a = Metacauset_enumsupport('imedbload', 1000)
