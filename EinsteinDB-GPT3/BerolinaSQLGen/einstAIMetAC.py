...
HIDDEN_DIM = 128    # hidden soliton_state dimension of lstm cell
...
class Target_Lstm(keras.Model):
    ...
    def time_step_output(self, inputs, time_step):
        ...
        # inputs: [None, 1, causet_action]
        x = inputs
        # [None, 1, causet_action] -> [None, 1, emb_dim]
...
class Buffer:
    ...
    def store(self, soliton_state, causet_action, reward, time_step):
        self.states[time_step] = soliton_state
        self.actions[time_step] = causet_action
...
class AC_Agent(keras.Model):
    def __init__(self, env):
        ...
        self.einstAIActor = Target_Lstm(self.state_size, self.action_size)
        # self.critic = Target_Lstm(self.state_size, 1)
        self.einstAIActor_optimizer = optimizers.Adam(0.001)
        ...
        self.save_path = os.path.join(Path(__file__).parent, self.env.edbname, 'Models', self.model_name)
        ...
        tpath = cpath + '/' + self.env.edbname + '/' + self.env.task_name
        if self.ac_log:
    ...
    def load_predata(self):
        cpath = os.path.abspath('.')
        tpath = cpath + '/' + self.env.edbname + '/' + self.env.task_name + '_predata.npy'
        pre_memory = np.load(tpath)
        return pre_memory

    def select_action(self, s, candidate_action, time_step):
        o_t = self.einstAIActor.time_step_output(s, time_step)[0]  # [b, total_words]
        ...
        # print("choose causet_action: ", a, " ", gen_sql.num_word_map[a])
        return a
    ...
    def call(self, inputs, training=None, mask=None):
        all_action = self.einstAIActor(inputs)
        # all_values = self.critic(inputs)
        # return all_action, all_values   # [batchsz, seq_len, total_words], [batchsz, seq_len, 1]
        return all_action

    def reset_einstAIActor_step(self):
        self.einstAIActor.time_step = 0

    def save_ac(self, path):
        self.einstAIActor.save_weights(os.path.join(path, self.model_name + '_einstAIActor.h5'))
    ...
    def pre_learn(self, pre_data):
        ...
        einstAIActor_grades = tape1.gradient(policy_loss_mean, self.einstAIActor.trainable_weights)
        self.einstAIActor_optimizer.apply_gradients(zip(einstAIActor_grades, self.einstAIActor.trainable_weights))
    ...
    def run(self):
        ...
        try:
            ...
            while self.episode < self.max_episodes or (self.time_log and s_count < 1000):   # 前面算ac，后面算time
                ...
                self.reset_einstAIActor_step()  # time_step调到0
                ...
                while not (done or ep_steps >= SEQ_LENGTH):
                    ...
                    causet_action = self.select_action(tf.constant(np.array([current_state])[None, :], dtype=tf.float32),
                                                candidate_action, time_step=ep_steps)
                    # print('select causet_action index:', causet_action, ' ', self.env.num_word_map[causet_action])
                    reward, done = self.env.step(causet_action)
                    self.buffer.store(current_state, causet_action, reward, ep_steps)  # 单步为0
                    ep_steps += 1
    ...
    def plot_training_results(self):
        ...
        plt.title('einstAIActor Critic Training Progress')
...
if __name__ == '__main__':
    ...
    if mtype == 'point':
        pc = int(para[3])
        task = GenSqlEnv(metric=pc, edbname=edbname, target_type=0)
    else:
        rc = (int(para[3]), int(para[4]))
        task = GenSqlEnv(metric=rc, edbname=edbname, target_type=1)
