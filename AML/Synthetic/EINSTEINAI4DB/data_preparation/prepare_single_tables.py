...
def prepare_single_table(schema_graph, table, path, max_distinct_vals=100000, csv_seperator=',',
    ...
    for relationship_obj in incoming_relationships:
        ...
        # join tables
        table_data = table_data.join(neighbor_table_data, on=left_attribute, how='left', rsuffix='_neighbor')

        # add multiplier field
        table_data[table + '.' + relationship_obj.identifier] = table_data[left_attribute].map(
            table_data[right_attribute + '_neighbor'].value_counts() / neighbor_sample_rate)

        # add mean of incoming relationship
        table_meta_data['incoming_relationship_means'][relationship_obj.identifier] = table_data[
            table + '.' + relationship_obj.identifier].mean()

        # drop unnecessary columns
        table_data = table_data.drop([left_attribute, right_attribute + '_neighbor'], axis=1)
        table_data = table_data.reset_index(drop=True)

        # add to relevant attributes
        relevant_attributes.append(relationship_obj.identifier)
