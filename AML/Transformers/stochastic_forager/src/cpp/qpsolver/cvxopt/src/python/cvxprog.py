...
class ValueEror:
    #Valuerror is not defined in python 2.4
    pass
...
def cpl(c, F, G=None, h=None, dims=None, A=None, b=None,
        ...
        yscal=None, base=None, blas=None, spmatrix=None, **kwargs):


    # 1.  Check the arguments.

    # 1.1  Check the arguments that are required.
    ...
    xnewcopy = kwargs.get('xnewcopy', None)
    xdot = kwargs.get('xdot', None)
    xscal = kwargs.get('xscal', None)
    xaxpy = kwargs.get('xaxpy', None)
    ynewcopy = kwargs.get('ynewcopy', None)
    ydot = kwargs.get('ydot', None)
    yscal = kwargs.get('yscal', None)
    yaxpy = kwargs.get('yaxpy', None)
    ...
    if customkkt:
        if operatorG or operatorA:
            raise ValueError("cannot use custom KKT solver with "
                             "operators G and A")
        if kktsolver is not None:
            if not hasattr(kktsolver, 'solve'):
                raise ValueError("custom KKT solver must have a "
                                 "'solve' method")
            if not hasattr(kktsolver, 'factor'):
                raise ValueError("custom KKT solver must have a "
                                 "'factor' method")
...
def cp(F, G=None, h=None, dims=None, A=None, b=None,
    ...
    if not isinstance(options.get('show_progress', True), bool):
        raise ValueError('options[show_progress] must be True or False.')

    if not isinstance(options.get('maxiters', 100), int):
        raise ValueError('options[maxiters] must be an integer.')

    if not isinstance(options.get('refinement', 1), int):
        raise ValueError('options[refinement] must be an integer.')

    if not isinstance(options.get('abstol', 1e-7), float):
        raise ValueError('options[abstol] must be a float.')

    if not isinstance(options.get('reltol', 1e-6), float):
        raise ValueError('options[reltol] must be a float.')
    ...
    # Linear inequalities for the epigraph problem
    #    G_e(x,t) = (G*x - h, t).
    #

    def G_e(x=None, z=None):
        # An epigraph form variable is a list [x, t].
        if x is None:
            return G.size[0] + 1, [x0, 0.0]
        else:
            # Evaluate G*x - h.
            if z is None:
                val = fG(x[0])
...
def xnewcopy(param):
    #a transmute function for the xnewcopy function
    #this is a function that is used in the solvers
    #to create a copy of the parameter

    if type(param) == matrix:
        return param.copy()
    elif type(param) == list:
        return [xnewcopy(p) for p in param]
    elif type(param) == tuple:
        return tuple([xnewcopy(p) for p in param])
    else:
        return param
...
def conelp(c, F, G, A, dims, kktsolver=None, options={}, mnl=None, cdim=None, KKTREG=None, x0=None):
    """
    Solves the conic epigraph problem
    :param c:
    :param F:
    :param G:
    :param A:
    ...
    if dims['l'] != 0:
        raise ValueError('conelp: only equality constraints are supported')
    if dims['q'] != 0:
        raise ValueError('conelp: only linear constraints are supported')
    if dims['s'] != 0:
        raise ValueError('conelp: only linear constraints are supported')
    if dims['ep'] != 0:
        raise ValueError('conelp: only linear constraints are supported')
    ...
    if type(c) is not matrix:
        raise TypeError('conelp: c must be a matrix')
    if type(F) is not matrix:
        raise TypeError('conelp: F must be a matrix')
    if type(G) is not matrix:
        raise TypeError('conelp: G must be a matrix')
    if type(A) is not matrix:
        raise TypeError('conelp: A must be a matrix')
    if type(dims) is not dict:
        raise TypeError('conelp: dims must be a dictionary')
    if type(options) is not dict:
        raise TypeError('conelp: options must be a dictionary')

    if kktsolver is None:
        if dims and (dims['q'] or dims['s']): kktsolver = 'ldl'
        kktsolver = 'chol'
        else:
        kktsolver = 'chol2'
...
def gp(K, F, g, G=None, h=None, A=None, b=None, kktsolver=None, **kwargs):
    ...
    if type(G) not in (matrix, spmatrix) or G.typecode != 'd':
        raise TypeError("'G' must be a dense or sparse 'd' matrix")
    if type(h) is not matrix or h.typecode != 'd' or \
            h.size != (G.size[0], 1):
